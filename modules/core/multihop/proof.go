...
// paths[len(paths)-1].EndpointB and all intermediate consensus states.
func GenerateMultiHopConsensusProof(paths []*ibctesting.Path) ([]*ConsStateProof, error) {
	if len(paths) < 2 {
		panic("paths must have at least two elements")
	}
...
func VerifyMultiHopConsensusStateProof(endpoint *ibctesting.Endpoint, proof []*ConsStateProof) error {
...
func VerifyMultiHopProofMembership(endpoint *ibctesting.Endpoint, proof []*ConsStateProof, value []byte) error {
	if len(proof) < 2 {
		return fmt.Errorf(
...
func VerifyMultiHopProofNonMembership(endpoint *ibctesting.Endpoint, proof []*ConsStateProof) error {
	if len(proof) < 2 {
		return fmt.Errorf(
			"proof must have at least two elements where the first one is the proof for the key and the rest are for the consensus states",
		)
