...
func defaultIdentifierValidator(id string, minimum, maximum int) error {
	...
	if len(id) < minimum || len(id) > maximum {
		return errorsmod.Wrapf(ErrInvalidID, "identifier %s has invalid length: %d, must be between %d-%d characters", id, len(id), minimum, maximum)
	}
