...
type Router struct {
	routes map[string]PacketModule
	sealed bool
}

func NewRouter() *Router {
	return &Router{
		routes: make(map[string]PacketModule),
	}
}
...
func (rtr *Router) AddRoute(module string, cbs PacketModule) *Router {
	if rtr.sealed {
...
func (rtr *Router) GetRoute(module string) (PacketModule, bool) {
	if !rtr.HasRoute(module) {
		return nil, false
	}
