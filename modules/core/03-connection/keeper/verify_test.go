	...
	for _, tc := range cases {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			err := suite.chainA.App.GetIBCKeeper().ConnectionKeeper.VerifyChannelState(
				suite.chainA.GetContext(), path.EndpointB.ChannelConfig.PortID, path.EndpointB.ChannelID,
				connection, malleateHeight(proofHeight, heightDiff), proof, channel,
			)
	...
	for _, tc := range cases {
		...
		suite.Run(tc.name, func() {
			...
			err = suite.chainB.App.GetIBCKeeper().ConnectionKeeper.VerifyPacketCommitment(
				suite.chainB.GetContext(), packet.GetSourcePort(), packet.GetSourceChannel(),
				connection, malleateHeight(proofHeight, heightDiff), proof, packet.GetSequence(), commitment,
			)
	...
	for _, tc := range cases {
		...
		suite.Run(tc.name, func() {
			...
			err = suite.chainA.App.GetIBCKeeper().ConnectionKeeper.VerifyPacketAcknowledgement(
				suite.chainA.GetContext(), packet.GetDestPort(), packet.GetDestChannel(),
				connection, malleateHeight(proofHeight, heightDiff), proof, packet.GetSequence(),
				ack.Acknowledgement(),
			)
