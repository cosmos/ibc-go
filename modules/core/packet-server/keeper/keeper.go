...
// SetCounterparty sets the Counterparty for a given channel identifier.
func (k *Keeper) SetCounterparty(ctx sdk.Context, channelID string, counterparty types.Counterparty) {
	bz := k.cdc.MustMarshal(&counterparty)
	k.ChannelStore(ctx, channelID).Set([]byte(types.CounterpartyKey), bz)
}
