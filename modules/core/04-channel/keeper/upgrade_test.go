	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			if tc.expError != nil {
				...
				expectedBlockTime := types.DefaultTimeout.Timestamp + uint64(suite.chainB.GetContext().BlockTime().UnixNano())
				suite.Require().Equal(expectedBlockTime, upgrade.Timeout.Timestamp)
