...
func (suite *KeeperTestSuite) TestChanUpgradeInit() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				suite.Require().NoError(path.EndpointA.UpdateChannel(func(channel *types.Channel) { channel.UpgradeSequence = 4 }))
				expSequence = 5
			},
			true,
		},
...
func (suite *KeeperTestSuite) TestChanUpgradeTry() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				suite.Require().NoError(path.EndpointA.UpdateChannel(func(channel *types.Channel) { channel.UpgradeSequence = 5 }))
			},
			nil,
		},
		{
		...
		},
		{
			...
			func() {
				suite.Require().NoError(path.EndpointB.UpdateChannel(func(channel *types.Channel) { channel.ConnectionHops = []string{"connection-100"} }))
			},
		...
		},
		{
			...
			func() {
				suite.Require().NoError(path.EndpointB.UpdateChannel(func(channel *types.Channel) { channel.Ordering = types.ORDERED }))
			},
		...
		},
		{
			...
			func() {
				suite.Require().NoError(path.EndpointB.UpdateChannel(func(channel *types.Channel) { channel.UpgradeSequence = 5 }))
			},
		...
		},
		{
			...
			func() {
				suite.Require().NoError(path.EndpointB.UpdateChannel(func(channel *types.Channel) { channel.UpgradeSequence = 4 }))
...
func (suite *KeeperTestSuite) TestChanUpgradeAck() {
	...
	testCases := []struct {
		...
		},
		{
			"success with later upgrade sequence",
			func() {
				suite.Require().NoError(path.EndpointA.UpdateChannel(func(channel *types.Channel) { channel.UpgradeSequence = 10 }))
				suite.Require().NoError(path.EndpointB.UpdateChannel(func(channel *types.Channel) { channel.UpgradeSequence = 10 }))
		...
		},
		{
			"fails due to proof verification failure, counterparty channel ordering does not match expected ordering",
			func() {
				suite.Require().NoError(path.EndpointA.UpdateChannel(func(channel *types.Channel) { channel.Ordering = types.ORDERED }))
			},
			commitmenttypes.ErrInvalidProof,
		},
		{
...
func (suite *KeeperTestSuite) TestChanUpgradeOpen() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				suite.Require().NoError(path.EndpointB.UpdateChannel(func(channel *types.Channel) { channel.UpgradeSequence-- }))
			},
...
func (suite *KeeperTestSuite) TestChanUpgradeCancel() {
	...
	for _, tc := range tests {
		...
		suite.Run(tc.name, func() {
			...
			suite.Require().NoError(path.EndpointA.UpdateChannel(func(channel *types.Channel) { channel.UpgradeSequence = 1 }))

			suite.Require().NoError(path.EndpointB.UpdateChannel(func(channel *types.Channel) { channel.UpgradeSequence = 2 }))
...
func (suite *KeeperTestSuite) TestChanUpgradeTimeout() {
	...
	testCases := []struct {
		...
		},
		{
			"connection not found",
			func() {
				suite.Require().NoError(path.EndpointA.UpdateChannel(func(channel *types.Channel) { channel.ConnectionHops[0] = ibctesting.InvalidID }))
			},
			connectiontypes.ErrConnectionNotFound,
		},
		{
		...
		},
		{
			...
			func() {
				suite.Require().NoError(path.EndpointB.UpdateChannel(func(channel *types.Channel) { channel.State = types.OPEN }))
		...
		},
		{
			...
			func() {
				suite.Require().NoError(path.EndpointB.UpdateChannel(func(channel *types.Channel) {
