...
import (
	...
	channeltypesv1 "github.com/cosmos/ibc-go/v11/modules/core/04-channel/types"
...
func (p Packet) ValidateBasic() error {
	if len(p.Payloads) != 1 {
		return errorsmod.Wrapf(ErrInvalidPayload, "payload length is %d, must be 1", len(p.Payloads))
	}
	...
	if totalPayloadsSize > channeltypesv1.MaximumPayloadsSize {
		return errorsmod.Wrapf(ErrInvalidPayload, "packet data bytes cannot exceed %d bytes", channeltypesv1.MaximumPayloadsSize)
	}
