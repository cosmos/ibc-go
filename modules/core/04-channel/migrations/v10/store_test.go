...
func (suite *MigrationsV10TestSuite) TestMigrateStore() {
	...
	// TODO: prove that the keys and values exist before Migrate

	storeService := runtime.NewKVStoreService(suite.chainA.GetSimApp().GetKey(ibcexported.StoreKey))
	err := v10.MigrateStore(ctx, storeService, cdc, suite.chainA.GetSimApp().IBCKeeper.ChannelKeeper)
	suite.Require().NoError(err)

	suite.assertChannelsUpgraded(ctx, suite.chainA.App.AppCodec(), storeService, suite.chainA.GetSimApp().IBCKeeper.ChannelKeeper)
	suite.assertNoUpgrades(ctx, storeService)
	suite.assertNoParms(ctx, storeService)
	suite.assertNoPruneSequences(ctx, storeService)
}

func (suite *MigrationsV10TestSuite) assertChannelsUpgraded(ctx sdk.Context, cdc codec.Codec, storeService corestore.KVStoreService, channelKeeper *keeper.Keeper) {
	...
	foundInKeeper := 0
	channelKeeper.IterateChannels(ctx, func(channel types.IdentifiedChannel) bool {
		suite.Require().NoError(channel.ValidateBasic())
		foundInKeeper++
		return false
	})

	suite.Require().Equal(foundWithOldType, foundInKeeper)
}
