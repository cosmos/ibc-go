...
// RegisterInterchainAccount registers a new interchain account. The MsgChannelOpenInitResponse is discarded.
...
// - The MsgChannelOpenInitResponse response is returned.
// - An error is also returned if the port identifier is already in use. Gaining access to interchain accounts whose channels
// have closed cannot be done with this function. A regular MsgChannelOpenInit must be used.
func (k Keeper) registerInterchainAccount(ctx sdk.Context, connectionID, owner, version string) (channeltypes.MsgChannelOpenInitResponse, error) {
	portID, err := icatypes.NewControllerPortID(owner)
	if err != nil {
		return channeltypes.MsgChannelOpenInitResponse{}, err
	}
	...
	if found {
		return channeltypes.MsgChannelOpenInitResponse{}, sdkerrors.Wrapf(icatypes.ErrActiveChannelAlreadySet, "existing active channel %s for portID %s on connection %s for owner %s", activeChannelID, portID, connectionID, owner)
	}

	switch {
	case k.portKeeper.IsBound(ctx, portID) && !k.IsBound(ctx, portID):
		return channeltypes.MsgChannelOpenInitResponse{}, sdkerrors.Wrapf(icatypes.ErrPortAlreadyBound, "another module has claimed capability for and bound port with portID: %s", portID)
	case !k.portKeeper.IsBound(ctx, portID):
		...
		if err := k.ClaimCapability(ctx, cap, host.PortPath(portID)); err != nil {
			return channeltypes.MsgChannelOpenInitResponse{}, sdkerrors.Wrapf(err, "unable to bind to newly generated portID: %s", portID)
		}
	}
	...
	if !ok {
		return channeltypes.MsgChannelOpenInitResponse{}, sdkerrors.Wrapf(sdkerrors.ErrInvalidType, "failed to covert %s message response to %s", reflect.TypeOf(firstMsgResponse.GetCachedValue()), reflect.TypeOf(&channeltypes.MsgChannelOpenInitResponse{}))
	}
	return *channelOpenInitResponse, nil
}
