	...
	testCases := []struct {
	...
	}{
		{
			...
			func(encoding string) {
				...
				proposal, err := govtypesv1.NewProposal([]sdk.Msg{getTestProposalMessage()}, govtypesv1.DefaultStartingProposalID, time.Now(), time.Now().Add(time.Hour), "test proposal", "title", "Description", interchainAccountAddr, govtypesv1.ProposalType_PROPOSAL_TYPE_STANDARD)
	...
	testCases := []struct {
	...
	}{
		{
			...
			func(icaAddress string) {
				proposal, err := govtypesv1.NewProposal([]sdk.Msg{getTestProposalMessage()}, govtypesv1.DefaultStartingProposalID, suite.chainA.GetContext().BlockTime(), suite.chainA.GetContext().BlockTime(), "test proposal", "title", "Description", interchainAccountAddr, govtypesv1.ProposalType_PROPOSAL_TYPE_STANDARD)
				suite.Require().NoError(err)
