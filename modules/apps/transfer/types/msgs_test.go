...
func TestMsgTransferValidation(t *testing.T) {
	testCases := []struct {
		...
		{"memo must be empty if forwarding unwind is set to true", types.NewMsgTransfer(validPort, validChannel, coins, sender, receiver, clienttypes.ZeroHeight(), 100, "memo", types.NewForwarding(true)), types.ErrInvalidMemo},
