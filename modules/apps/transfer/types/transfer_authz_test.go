...
func TestTransferAuthorization(t *testing.T) {
	...
	authorization := NewTransferAuthorization([]string{sourcePort}, []string{sourceChannel}, []sdk.Coins{coins1000})
	...
	sendAuth := NewTransferAuthorization([]string{sourcePort}, []string{sourceChannel}, []sdk.Coins{coins500})
	...
	require.NoError(t, err)
	require.NotNil(t, resp.Updated)
	require.Equal(t, resp.Updated, NewTransferAuthorization([]string{sourcePort2}, []string{sourceChannel2}, []sdk.Coins{coins1000}))
	require.False(t, resp.Delete)
}
