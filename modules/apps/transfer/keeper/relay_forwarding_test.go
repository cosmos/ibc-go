...
func (suite *ForwardingTestSuite) setupFeeForwardingPaths() (pathAtoB, pathBtoC *ibctesting.Path) {
	pathAtoB = ibctesting.NewTransferPath(suite.chainA, suite.chainB)
	pathBtoC = ibctesting.NewTransferPath(suite.chainB, suite.chainC)

	pathAtoB.EndpointA.ChannelConfig.Version = string(feetypes.ModuleCdc.MustMarshalJSON(&feetypes.Metadata{FeeVersion: feetypes.Version, AppVersion: "ics20-2"}))
	pathAtoB.EndpointB.ChannelConfig.Version = string(feetypes.ModuleCdc.MustMarshalJSON(&feetypes.Metadata{FeeVersion: feetypes.Version, AppVersion: "ics20-2"}))

	pathBtoC.EndpointA.ChannelConfig.Version = string(feetypes.ModuleCdc.MustMarshalJSON(&feetypes.Metadata{FeeVersion: feetypes.Version, AppVersion: "ics20-2"}))
	pathBtoC.EndpointB.ChannelConfig.Version = string(feetypes.ModuleCdc.MustMarshalJSON(&feetypes.Metadata{FeeVersion: feetypes.Version, AppVersion: "ics20-2"}))
...
// TestSuccessfulForward tests a successful transfer from A to C through B.
func (suite *ForwardingTestSuite) TestSuccessfulForwardFee() {
	/*
		Given the following topology:
		chain A (channel 0) -> (channel-0) chain B (channel-1) -> (channel-0) chain C
		stake                  transfer/channel-0/stake           transfer/channel-0/transfer/channel-0/stake
	...
	*/
	...
	pathAtoB, pathBtoC := suite.setupFeeForwardingPaths()

	suite.chainB.GetSimApp().IBCFeeKeeper.SetCounterpartyPayeeAddress(suite.chainB.GetContext(), suite.chainB.SenderAccounts[0].SenderAccount.GetAddress().String(), ibctesting.TestAccAddress, pathAtoB.EndpointB.ChannelID)
	suite.chainC.GetSimApp().IBCFeeKeeper.SetCounterpartyPayeeAddress(suite.chainC.GetContext(), suite.chainC.SenderAccounts[0].SenderAccount.GetAddress().String(), ibctesting.TestAccAddress, pathBtoC.EndpointB.ChannelID)
	...
	successAck := feetypes.NewIncentivizedAcknowledgement(ibctesting.TestAccAddress, channeltypes.NewResultAcknowledgement([]byte{byte(1)}).Acknowledgement(), true)
	successAckBz := channeltypes.CommitAcknowledgement(successAck.Acknowledgement())
	ackOnC := suite.chainC.GetAcknowledgement(packetFromBtoC)
