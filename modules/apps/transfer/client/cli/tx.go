...
// NewTransferMultiDenomTxCmd returns the command to create a NewMsgTransfer multidenom transaction
func NewTransferMultiDenomTxCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "transfer-multi-denom [src-port] [src-channel] [receiver] [amount]",
		Short: "Transfer multiple fungible tokens in one packet through IBC",
		Long: strings.TrimSpace(`Transfer multiple fungible tokens in one packet through IBC. Timeouts can be specified
as absolute or relative using the "absolute-timeouts" flag. Timeout height can be set by passing in the height string
in the form {revision}-{height} using the "packet-timeout-height" flag. Relative timeout height is added to the block
height queried from the latest consensus state corresponding to the counterparty channel. Relative timeout timestamp 
is added to the greater value of the local clock time and the block timestamp queried from the latest consensus state 
corresponding to the counterparty channel. Any timeout set to 0 is disabled.`),
		Example: fmt.Sprintf("%s tx ibc-transfer transfer [src-port] [src-channel] [receiver] [amount in sdk.Coins]", version.AppName),
		Args:    cobra.ExactArgs(4),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
