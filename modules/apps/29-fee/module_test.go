...
func (suite *FeeTestSuite) TestOnChanOpenInit() {
	...
	testCases := []struct {
		...
		},
		{
			"capability already claimed", func() {
				err := suite.chainA.GetSimApp().ScopedFeeKeeper.ClaimCapability(suite.chainA.GetContext(), chanCap, host.ChannelCapabilityPath(path.EndpointA.ChannelConfig.PortID, path.EndpointA.ChannelID))
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			path = NewFeePath(suite.chainA, suite.chainB)
			...
			module, _, err := suite.chainA.App.GetIBCKeeper().PortKeeper.LookupModuleByPort(suite.chainA.GetContext(), ibctesting.FeePort)
			suite.Require().NoError(err)

			chanCap, err = suite.chainA.App.GetScopedIBCKeeper().NewCapability(suite.chainA.GetContext(), host.ChannelCapabilityPath(ibctesting.FeePort, path.EndpointA.ChannelID))
			suite.Require().NoError(err)
