...
func (im IBCMiddleware) OnChanUpgradeTry(ctx sdk.Context, order channeltypes.Order, connectionHops []string, portID, channelID string, counterpartyVersion string) (string, error) {
	return im.app.OnChanUpgradeTry(ctx, order, connectionHops, portID, channelID, counterpartyVersion)
}
