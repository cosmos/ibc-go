...
func (suite *KeeperTestSuite) TestPayPacketFeeAsync() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				packet = channeltypes.NewPacket(ibctesting.MockPacketData, sequence, suite.path.EndpointA.ChannelConfig.PortID, suite.path.EndpointA.ChannelID, suite.path.EndpointB.ChannelConfig.PortID, suite.path.EndpointB.ChannelID, timeout)
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			timeout := channeltypes.Timeout{
				Height:    clienttypes.NewHeight(clienttypes.ParseChainID(suite.chainB.ChainID), 100),
				Timestamp: 0,
			}
			...
			suite.Require().NoError(err)
			packetID := channeltypes.NewPacketID(suite.path.EndpointA.ChannelConfig.PortID, suite.path.EndpointA.ChannelID, sequence)
			packet = channeltypes.NewPacket(ibctesting.MockPacketData, packetID.Sequence, packetID.PortId, packetID.ChannelId, suite.path.EndpointB.ChannelConfig.PortID, suite.path.EndpointB.ChannelID, timeout)
