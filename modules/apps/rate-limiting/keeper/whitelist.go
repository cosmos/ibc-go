...
func (k Keeper) SetWhitelistedAddressPair(ctx sdk.Context, whitelist types.WhitelistedAddressPair) {
	...
	store := prefix.NewStore(adapter, []byte(types.AddressWhitelistKeyPrefix))
	key := types.KeyAddressWhitelist(whitelist.Sender, whitelist.Receiver)
...
func (k Keeper) RemoveWhitelistedAddressPair(ctx sdk.Context, sender, receiver string) {
	adapter := runtime.KVStoreAdapter(k.storeService.OpenKVStore(ctx))
	store := prefix.NewStore(adapter, []byte(types.AddressWhitelistKeyPrefix))
	key := types.KeyAddressWhitelist(sender, receiver)
	store.Delete(key)
}
