...
func (s *CallbacksTestSuite) TestNewIBCMiddleware() {
	testCases := []struct {
	...
	}{
		{
			"success",
			func() {
				_ = ibccallbacks.NewIBCMiddleware(ibcmock.IBCModule{}, s.chainA.GetSimApp().GetIBCKeeper().ChannelKeeper, s.chainA.ContractKeeper, maxCallbackGas)
			},
			nil,
		},
		{
			...
			func() {
				_ = ibccallbacks.NewIBCMiddleware(nil, s.chainA.GetSimApp().GetIBCKeeper().ChannelKeeper, s.chainA.ContractKeeper, maxCallbackGas)
			},
		...
		},
		{
			...
			func() {
				_ = ibccallbacks.NewIBCMiddleware(ibcmock.IBCModule{}, nil, s.chainA.ContractKeeper, maxCallbackGas)
			},
