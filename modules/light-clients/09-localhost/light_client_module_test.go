	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				...
				merklePath := api.NewMerklePath(host.FullClientStatePath(exported.LocalhostClientID))
				merklePath, err := api.ApplyPrefix(suite.chain.GetPrefix().Bytes(), merklePath)
		...
		},
		{
			...
			func() {
				...
				merklePath := api.NewMerklePath(host.ConnectionPath(exported.LocalhostConnectionID))
				merklePath, err := api.ApplyPrefix(suite.chain.GetPrefix().Bytes(), merklePath)
				suite.Require().NoError(err)
		...
		},
		{
			...
			func() {
				...
				merklePath := api.NewMerklePath(host.ChannelPath(mock.PortID, ibctesting.FirstChannelID))
				merklePath, err := api.ApplyPrefix(suite.chain.GetPrefix().Bytes(), merklePath)
				suite.Require().NoError(err)
		...
		},
		{
			...
			func() {
				...
				merklePath := api.NewMerklePath(host.NextSequenceRecvPath(mock.PortID, ibctesting.FirstChannelID))
				merklePath, err := api.ApplyPrefix(suite.chain.GetPrefix().Bytes(), merklePath)
				suite.Require().NoError(err)
		...
		},
		{
			...
			func() {
				...
				merklePath := api.NewMerklePath(host.PacketCommitmentPath(packet.GetSourcePort(), packet.GetSourceChannel(), packet.GetSequence()))
				merklePath, err := api.ApplyPrefix(suite.chain.GetPrefix().Bytes(), merklePath)
				suite.Require().NoError(err)
		...
		},
		{
			...
			func() {
				...
				merklePath := api.NewMerklePath(host.PacketAcknowledgementPath(mock.PortID, ibctesting.FirstChannelID, 1))
				merklePath, err := api.ApplyPrefix(suite.chain.GetPrefix().Bytes(), merklePath)
				suite.Require().NoError(err)
		...
		},
		{
			...
			func() {
				path = api.NewMerklePath("ibc", "test", "key")
			},
			false,
		},
		{
			...
			func() {
				merklePath := api.NewMerklePath(host.PacketAcknowledgementPath(mock.PortID, ibctesting.FirstChannelID, 100))
				merklePath, err := api.ApplyPrefix(suite.chain.GetPrefix().Bytes(), merklePath)
				suite.Require().NoError(err)
...
func (suite *LocalhostTestSuite) TestVerifyNonMembership() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				merklePath := api.NewMerklePath(host.PacketReceiptPath(mock.PortID, ibctesting.FirstChannelID, 1))
				merklePath, err := api.ApplyPrefix(suite.chain.GetPrefix().Bytes(), merklePath)
				suite.Require().NoError(err)
