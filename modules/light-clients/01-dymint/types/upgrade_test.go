...
func (suite *DymintTestSuite) TestVerifyUpgrade() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			setup: func() {
				...
				lastHeight = clienttypes.NewHeight(0, uint64(dymintChain.GetContext().BlockHeight()+1))

				// zero custom fields and store in upgrade store
				dymintChain.GetSimApp().UpgradeKeeper.SetUpgradedClient(dymintChain.GetContext(), int64(lastHeight.GetRevisionHeight()), upgradedClientBz)
				...
				cs, found := dymintCounterpartyChain.App.GetIBCKeeper().ClientKeeper.GetClientState(dymintCounterpartyChain.GetContext(), endpoint.ClientID)
				suite.Require().True(found)

				proofUpgradedClient, _ = dymintChain.QueryUpgradeProof(upgradetypes.UpgradedClientKey(int64(lastHeight.GetRevisionHeight())), cs.GetLatestHeight().GetRevisionHeight())
				proofUpgradedConsState, _ = dymintChain.QueryUpgradeProof(upgradetypes.UpgradedConsStateKey(int64(lastHeight.GetRevisionHeight())), cs.GetLatestHeight().GetRevisionHeight())
			},
			expPass: true,
		},
		{
...

		{
			...
			setup: func() {
				...
				lastHeight = clienttypes.NewHeight(0, uint64(dymintChain.GetContext().BlockHeight()+10))
		...
		},
		{
			...
			setup: func() {
				...
				dymintCounterpartyChain.App.GetIBCKeeper().ClientKeeper.SetClientState(dymintCounterpartyChain.GetContext(), endpoint.ClientID, tmClient)
			},
			expPass: false,
		},
		{
		...
		},
		{
			...
			setup: func() {
				// upgrade Height is at next block
				lastHeight = clienttypes.NewHeight(0, uint64(dymintChain.GetContext().BlockHeight()+100))
		...
		},
		{
			...
			setup: func() {
				...
				dymintCounterpartyChain.ExpireClient(ubdPeriod)
	...
	for _, tc := range testCases {
		...
		upgradedConsStateBz, err = clienttypes.MarshalConsensusState(dymintCounterpartyChain.App.AppCodec(), upgradedConsState)
		suite.Require().NoError(err)

		tc.setup()

		cs := dymintCounterpartyChain.GetClientState(endpoint.ClientID)
		...
		clientState, consensusState, err := cs.VerifyUpgradeAndUpdateState(
			dymintCounterpartyChain.GetContext(),
			suite.cdc,
			clientStore,
			upgradedClient,
			upgradedConsState,
