...
func (Keeper) importWasmCode(ctx sdk.Context, code []byte) error {
	var err error
	...
	if err != nil {
		return errorsmod.Wrap(err, "wasm bytecode checksum failed")
	}
	...
	if !bytes.Equal(vmChecksum, checksum) {
		return errorsmod.Wrapf(types.ErrInvalidChecksum, "expected %s, got %s", hex.EncodeToString(checksum), hex.EncodeToString(vmChecksum))
	}

	// pin the code to the vm in-memory cache
	if err := ibcwasm.GetVM().Pin(vmChecksum); err != nil {
		return errorsmod.Wrapf(err, "failed to pin contract with checksum (%s) to vm cache", hex.EncodeToString(vmChecksum))
	}

	// store the checksum
	err = ibcwasm.Checksums.Set(ctx, checksum)
	if err != nil {
		return errorsmod.Wrap(err, "failed to store checksum")
	}
