...
func (k Keeper) storeWasmCode(ctx sdk.Context, code []byte) ([]byte, error) {
	...
	codeHash := generateWasmChecksum(code)
	if types.HasChecksum(ctx, codeHash) {
	...
	if !bytes.Equal(vmCodeHash, codeHash) {
		return nil, errorsmod.Wrapf(types.ErrInvalidChecksum, "expected %s, got %s", hex.EncodeToString(codeHash), hex.EncodeToString(vmCodeHash))
	}

	// pin the code to the vm in-memory cache
	if err := k.wasmVM.Pin(vmCodeHash); err != nil {
		return nil, errorsmod.Wrapf(err, "failed to pin contract with checksum (%s) to vm cache", hex.EncodeToString(vmCodeHash))
	}
