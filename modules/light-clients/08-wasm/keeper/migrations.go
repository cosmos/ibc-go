...
// MigrateWasmStore migrates the wasm store from using a single key to
...
func (m Migrator) MigrateWasmStore(ctx sdk.Context) error {
	...
	for _, hash := range checksums {
		ibcwasm.Checksums.Set(ctx, hash)
	}

	// delete the previously stored checksums
	m.deleteChecksums(ctx)
...
func (m Migrator) deleteChecksums(ctx sdk.Context) {
	store := m.keeper.storeService.OpenKVStore(ctx)
	store.Delete([]byte(types.KeyChecksums))
}
