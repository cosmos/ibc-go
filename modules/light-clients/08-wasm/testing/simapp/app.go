	...
	*/
	...
	if mockVM != nil {
		...
		app.WasmClientKeeper = wasmkeeper.NewKeeperWithVM(
			...
			authtypes.NewModuleAddress(govtypes.ModuleName).String(), mockVM, app.GRPCQueryRouter(), nil,
		)
	} else {
		app.WasmClientKeeper = wasmkeeper.NewKeeperWithConfig(
			appCodec, runtime.NewKVStoreService(keys[wasmtypes.StoreKey]), app.IBCKeeper.ClientKeeper,
			authtypes.NewModuleAddress(govtypes.ModuleName).String(), wasmConfig, app.GRPCQueryRouter(), nil,
		)
	}
