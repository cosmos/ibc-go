...
func (suite *TypesTestSuite) TestUpdateState() {
	...
	testCases := []struct {
		...
		},
		{
			...
			errorsmod.Wrap(types.ErrWasmContractCallFailed, errorsmod.Wrap(errorsmod.Wrapf(types.ErrRetrieveClientID, "prefix does not contain a valid clientID: %s", errorsmod.Wrapf(types.ErrInvalidWasmClientID, "invalid client identifier %s", ibctesting.InvalidID)), "failed to retrieve clientID for wasm contract call").Error()),
			nil,
			nil,
		},
		{
