...
func (suite *TypesTestSuite) TestUpdateState() {
	...
	testCases := []struct {
		...
		},
		{
			...
			errorsmod.Wrapf(types.ErrWasmContractCallFailed, "call to wasm contract failed: %v", errorsmod.Wrapf(errorsmod.Wrapf(types.ErrRetrieveClientID, "prefix does not contain a %s clientID", exported.Wasm), "failed to retrieve clientID for wasm contract call")),
			nil,
			nil,
		},
		{
