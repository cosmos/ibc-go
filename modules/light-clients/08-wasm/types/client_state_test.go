...
func (suite *TypesTestSuite) TestGetTimestampAtHeight() {
	...
	testCases := []struct {
		...
		},
		{
			...
			errorsmod.Wrapf(errorsmod.Wrapf(types.ErrWasmContractCallFailed, "query to wasm contract failed: %v", wasmtesting.ErrMockContract), "height (%s)", height),
		},
		{
...
func (suite *TypesTestSuite) TestInitialize() {
	...
	testCases := []struct {
		...
		},
		{
			...
			errorsmod.Wrapf(types.ErrWasmContractCallFailed, "call to wasm contract failed: %v", errorsmod.Wrapf(errorsmod.Wrapf(types.ErrRetrieveClientID, "prefix does not contain a %s clientID", exported.Wasm), "failed to retrieve clientID for wasm contract instantiation")),
		},
		{
...
func (suite *TypesTestSuite) TestVerifyNonMembership() {
	...
	testCases := []struct {
		...
		},
		{
			...
			errorsmod.Wrapf(types.ErrWasmContractCallFailed, "call to wasm contract failed: %v", commitmenttypes.ErrInvalidProof),
		},
		{
			"proof height greater than client state latest height",
			func() {
