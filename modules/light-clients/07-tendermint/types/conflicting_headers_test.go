...
func (suite *TendermintTestSuite) TestConflictingHeaders() {
	signers := []tmtypes.PrivValidator{suite.privVal}
	heightMinus1 := clienttypes.NewHeight(0, height.RevisionHeight-1)

	misbehaviour := &types.ConflictingHeaders{
...
func (suite *TendermintTestSuite) TestConflictingHeadersValidateBasic() {
	...
	testCases := []struct {
		name                       string
		misbehaviour               *types.ConflictingHeaders
		malleateConflictingHeaders func(misbehaviour *types.ConflictingHeaders) error
		expPass                    bool
	}{
		{
			"valid fork misbehaviour, two headers at same height have different time",
			&types.ConflictingHeaders{
				Header1:  suite.header,
			...
			func(misbehaviour *types.ConflictingHeaders) error { return nil },
			true,
		},
		{
		...
		},
		{
			...
			types.NewConflictingHeaders(clientID, nil, suite.header),
			func(m *types.ConflictingHeaders) error { return nil },
			false,
		},
		{
			"misbehaviour Header2 is nil",
			types.NewConflictingHeaders(clientID, suite.header, nil),
			func(m *types.ConflictingHeaders) error { return nil },
			false,
		},
		{
		...
		},
		{
			...
			func(misbehaviour *types.ConflictingHeaders) error {
	...
	for i, tc := range testCases {
		...
		err := tc.malleateConflictingHeaders(tc.misbehaviour)
		suite.Require().NoError(err)
