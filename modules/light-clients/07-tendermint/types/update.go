...
func (cs ClientState) CheckHeaderAndUpdateState(
	...
	if err := cs.VerifyHeader(ctx, cdc, clientStore, header); err != nil {
		// NB: We're not wrapping the error here for consistency with previous behaviour. Outer fn is to be removed later.
		return nil, nil, err
	}

	misbehaving, err := cs.CheckForMisbehaviour(ctx, cdc, clientStore, header)
	if err != nil {
		return nil, nil, err
	}
	if misbehaving {
...
func (cs ClientState) VerifyHeader(
	...
	if err := checkTrustedHeader(tmHeader, trustedConsState); err != nil {
		return err
	}
	...
	if tmHeader.GetHeight().GetRevisionNumber() != tmHeader.TrustedHeight.RevisionNumber {
		return sdkerrors.Wrapf(
			...
			tmHeader.GetHeight().GetRevisionNumber(), tmHeader.TrustedHeight.RevisionNumber,
		)
	}

	tmTrustedValidators, err := tmtypes.ValidatorSetFromProto(tmHeader.TrustedValidators)
	if err != nil {
		return sdkerrors.Wrap(err, "trusted validator set in not tendermint validator set type")
	}

	tmSignedHeader, err := tmtypes.SignedHeaderFromProto(tmHeader.SignedHeader)
	if err != nil {
		return sdkerrors.Wrap(err, "signed header in not tendermint signed header type")
	}

	tmValidatorSet, err := tmtypes.ValidatorSetFromProto(tmHeader.ValidatorSet)
	if err != nil {
	...
	if tmHeader.GetHeight().LTE(tmHeader.TrustedHeight) {
		return sdkerrors.Wrapf(
			...
			"header height ≤ consensus state height (%s ≤ %s)", tmHeader.GetHeight(), tmHeader.TrustedHeight,
		)
	}
	...
	if clienttypes.IsRevisionFormat(chainID) {
		chainID, _ = clienttypes.SetRevisionNumber(chainID, tmHeader.GetHeight().GetRevisionNumber())
	}
	...
	err = light.Verify(
		...
		cs.TrustingPeriod, ctx.BlockTime(), cs.MaxClockDrift, cs.TrustLevel.ToTendermint(),
	)
	if err != nil {
...
func (cs ClientState) CheckForMisbehaviour(
	...
	misbehaviour, ok := header.(*DuplicateHeaderHeader)
	if ok {
		return cs.checkDuplicateHeaderHeader(ctx, cdc, clientStore, misbehaviour)
	}
...
//
// Pruning:
// UpdateState will additionally retrieve the earliest consensus state for this clientID and check if it is expired. If it is,
// that consensus state will be pruned from store along with all associated metadata. This will prevent the client store from
// becoming bloated with expired consensus states that can no longer be used for updates and packet verification.
func (cs ClientState) UpdateState(
	...
	setConsensusMetadata(ctx, clientStore, tmHeader.GetHeight())
