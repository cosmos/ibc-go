	...
	testCases := []struct {
		...
		},
		{
			"valid misbehaviour with different trusted heights", func() {
				trustedHeight1 := path.EndpointA.GetClientState().(*ibctm.ClientState).LatestHeight
				...
				trustedHeight2 := path.EndpointA.GetClientState().(*ibctm.ClientState).LatestHeight
