...
func (suite *TendermintTestSuite) TestExportMetadata() {
	...
	initIteration := tendermint.GetIterationKey(clientStore, height)
	suite.Require().NotEqual(0, len(initIteration))
	initProcessedTime, found := tendermint.GetProcessedTime(clientStore, height)
	suite.Require().True(found)
	initProcessedHeight, found := tendermint.GetProcessedHeight(clientStore, height)
	suite.Require().True(found)
	...
	suite.Require().Equal(tendermint.ProcessedHeightKey(height), gm[0].GetKey(), "metadata has unexpected key")
	...
	suite.Require().Equal(tendermint.ProcessedTimeKey(height), gm[1].GetKey(), "metadata has unexpected key")
	suite.Require().Equal(initProcessedTime, sdk.BigEndianToUint64(gm[1].GetValue()), "metadata has unexpected value")

	suite.Require().Equal(tendermint.IterationKey(height), gm[2].GetKey(), "metadata has unexpected key")
	...
	iteration := tendermint.GetIterationKey(clientStore, updateHeight)
	suite.Require().NotEqual(0, len(initIteration))
	processedTime, found := tendermint.GetProcessedTime(clientStore, updateHeight)
	suite.Require().True(found)
	processedHeight, found := tendermint.GetProcessedHeight(clientStore, updateHeight)
	suite.Require().True(found)
	...
	suite.Require().Equal(tendermint.ProcessedHeightKey(updateHeight), gm[2].GetKey(), "metadata has unexpected key")
	...
	suite.Require().Equal(tendermint.ProcessedTimeKey(updateHeight), gm[3].GetKey(), "metadata has unexpected key")
	suite.Require().Equal(processedTime, sdk.BigEndianToUint64(gm[3].GetValue()), "metadata has unexpected value")

	// check iteration keys
	suite.Require().Equal(tendermint.IterationKey(height), gm[4].GetKey(), "metadata has unexpected key")
	suite.Require().Equal(initIteration, gm[4].GetValue(), "metadata has unexpected value")

	suite.Require().Equal(tendermint.IterationKey(updateHeight), gm[5].GetKey(), "metadata has unexpected key")
