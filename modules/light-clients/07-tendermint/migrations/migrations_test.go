...
func (suite *MigrationsTestSuite) TestPruneExpiredConsensusStates() {
	...
	for _, path := range paths {
		...
		pruneHeights = append(pruneHeights, path.EndpointA.GetClientState().(*ibctm.ClientState).LatestHeight)
	...
	for _, path := range paths {
		...
		err := path.EndpointA.UpdateClient()
		suite.Require().NoError(err)
		unexpiredHeights = append(unexpiredHeights, path.EndpointA.GetClientState().(*ibctm.ClientState).LatestHeight)
