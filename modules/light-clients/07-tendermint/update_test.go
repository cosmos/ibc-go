	...
	testCases := []struct {
	...
	}{
		{
			"success with height later than latest height", func() {
				...
				suite.Require().True(path.EndpointA.GetClientState().(*ibctm.ClientState).LatestHeight.LT(tmHeader.GetHeight()))
			},
			func() {
				tmHeader, ok := clientMessage.(*ibctm.Header)
				suite.Require().True(ok)
		...
		},
		{
			"success with height earlier than latest height", func() {
				...
				tmHeader, ok := clientMessage.(*ibctm.Header)
				suite.Require().True(ok)
				suite.Require().True(path.EndpointA.GetClientState().(*ibctm.ClientState).LatestHeight.GT(tmHeader.GetHeight()))
		...
		},
		{
			"success with duplicate header", func() {
				...
				tmHeader, ok := clientMessage.(*ibctm.Header)
				suite.Require().True(ok)
				suite.Require().Equal(path.EndpointA.GetClientState().(*ibctm.ClientState).LatestHeight, tmHeader.GetHeight())
		...
		},
		{
			"success with pruned consensus state", func() {
				...
				err := path.EndpointA.UpdateClient()
				suite.Require().NoError(err)
				pruneHeight = path.EndpointA.GetClientState().(*ibctm.ClientState).LatestHeight
...
func (suite *TendermintTestSuite) TestPruneConsensusState() {
	...
	err := path.EndpointA.UpdateClient()
	suite.Require().NoError(err)
	expiredHeight := path.EndpointA.GetClientState().(*ibctm.ClientState).LatestHeight
