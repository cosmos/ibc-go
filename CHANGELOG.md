...
* (core/04-channel/v2) [\#9078](https://github.com/cosmos/ibc-go/pull/9078) Use a fresh cached context for the application callbacks in `MsgRecvPacket` so the core `recv_packet` events are no longer emitted twice on success, and are no longer re-emitted as error events when an application fails.
