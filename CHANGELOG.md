...
* (apps/callbacks) [\#9079](https://github.com/cosmos/ibc-go/pull/9079) Pass the raw application acknowledgement bytes to the destination callback in the IBC v2 asynchronous `WriteAcknowledgement` path, matching the synchronous `OnRecvPacket` path (previously the contract received a protobuf-encoded `channeltypesv2.Acknowledgement` wrapper).
