...
1. Create a governance proposal with the [`MsgIBCSoftwareUpgrade`](https://github.com/cosmos/ibc-go/blob/main/docs/ibc/proto-docs.md#msgibcsoftwareupgrade) message which contains an `UpgradePlan` and a new IBC ClientState in the `UpgradedClientState` field. Note that the `UpgradePlan` must specify an upgrade height **only** (no upgrade time), and the `ClientState` should only include the fields common to all valid clients and zero out any client-customizable fields (such as TrustingPeriod).
