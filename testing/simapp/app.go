...
func NewSimappWithOptions(
	logger log.Logger,
	db dbm.DB,
	traceStore io.Writer,
	loadLatest bool,
	appOpts servertypes.AppOptions,
	simAppOptions ...func(*SimApp),
) *SimApp{
	app := NewSimApp(logger, db, traceStore, loadLatest, appOpts)
	for _, option := range simAppOptions {
		option(app)
	}
	return app
}
...
func NewSimApp(
	...
	*/
	...
	app.ModuleManager = module.NewManager(
		...
		transfer.NewAppModule(&app.TransferKeeper), // We pass a reference to the transfer keeper so that we can modify it in the callbacks module.
