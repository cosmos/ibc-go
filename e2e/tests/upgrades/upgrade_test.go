...
func (s *UpgradeTestSuite) TestClientUpgrade() {
	t := s.T()
	testCfg := testconfig.FromEnv()
	...
	t.Run("upgrade chainA", func(t *testing.T) {
		s.UpgradeChain(ctx, chainA, chainAUpgradeProposalWallet, "e2e-client-upgrade", testCfg.ChainAConfig.Tag, testCfg.UpgradeTag)
	})
	//
	//t.Run("restart relayer", func(t *testing.T) {
	//	s.StopRelayer(ctx, relayer)
