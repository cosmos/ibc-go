...
func (s *InterchainAccountsTestSuite) TestMsgSendTx_IncorrectOwner() {
	t := s.T()
	ctx := context.TODO()
	...
	// setup 3 accounts: controller account on chain A, a non controller account on chain A
	// and a third chain B account. host account will be created when the ICA is registered
	controllerAccount := s.CreateUserOnChainA(ctx, testvalues.StartingTokenAmount)
	nonControllerChainAAccount := s.CreateUserOnChainA(ctx, testvalues.StartingTokenAmount)
	chainBAccount := s.CreateUserOnChainB(ctx, testvalues.StartingTokenAmount)
	var hostAccount string
	...
	t.Run("interchain account executes a bank transfer on behalf of the corresponding owner account", func(t *testing.T) {
		...
		t.Run("broadcast MsgSendTx", func(t *testing.T) {
			...
			msgSendTx := controllertypes.NewMsgSendTx(nonControllerChainAAccount.Bech32Address(chainA.Config().Bech32Prefix), ibctesting.FirstConnectionID, uint64(time.Hour.Nanoseconds()), packetData)
		...
		t.Run("verify tokens are not transferred", func(t *testing.T) {
			balance, err := chainB.GetBalance(ctx, chainBAccount.Bech32Address(chainB.Config().Bech32Prefix), chainB.Config().Denom)
			s.Require().NoError(err)
