...
func (s *IncentivizedTransferTestSuite) TestMsgPayPacketFee_InvalidReceiverAccount() {
	...
	t.Run("send IBC transfer", func(t *testing.T) {
		msgTransfer := transfertypes.NewMsgTransfer(channelA.PortID, channelA.ChannelID, []sdk.Coin{transferAmount}, chainAWallet.FormattedAddress(), testvalues.InvalidAddress, s.GetTimeoutHeight(ctx, chainB), 0, "")
...
func (s *IncentivizedTransferTestSuite) TestMultiMsg_MsgPayPacketFeeSingleSender() {
	...
	msgTransfer := transfertypes.NewMsgTransfer(channelA.PortID, channelA.ChannelID, []sdk.Coin{transferAmount}, chainAWallet.FormattedAddress(), chainBWallet.FormattedAddress(), s.GetTimeoutHeight(ctx, chainB), 0, "")
